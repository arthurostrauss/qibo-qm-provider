...
from qibo_qm_provider.qibolab_bridge import IQCCQmController
...
def test_resolve_controller_class_falls_back_without_iqcc_cloud_client():
    ...
    back to a plain ``QmController`` rather than raising -- the real
    connection failure belongs at ``connect()`` time, not here."""
    assert "iqcc_cloud_client" not in sys.modules
    assert _resolve_controller_class(_iqcc_machine()) is QmController


def test_resolve_controller_class_ignores_missing_qmm_class():
    machine = FluxTunableQuam()
    machine.network = {"host": "1.2.3.4"}
    assert _resolve_controller_class(machine) is QmController
